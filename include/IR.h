...
public:
    ...
    // 创建结构体类型
    static Type *getStructTy(const std::string &name, const std::vector<Type *> &members)
    {
        auto ty = new Type(StructTyID, "%struct." + name);
        ty->memberTypes = members;
        return ty;
    }
...
public:
    LoadInst(ValuePtr ptr, std::string name)
        : Instruction(ptr->getType()->isPointerTy() ? static_cast<Type *>(ptr->getType())->elementType : Type::getInt32Ty(),
                      name, "load",
                      (ptr->getType()->isPointerTy() ? static_cast<Type *>(ptr->getType())->elementType->toString() : "i32") +
...
public:
    ...
    Function(Type *retType, const std::string &name)
        : Value(retType, "@" + name), returnType(retType)
    { // 在构造函数中初始化 returnType
      // Entry block created manually by Generator
    }
